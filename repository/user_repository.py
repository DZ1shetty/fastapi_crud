...
class UserRepository:
    ...
    def get_all_users(self):
        with self._get_connection() as connection:
            rows = connection.execute('SELECT id, name, email FROM users ORDER BY id').fetchall()
    ...
    def update_user(self, user_id: int, name: str, email: str) -> bool:
        with self._get_connection() as connection:
            cursor = connection.execute('UPDATE users SET name = ?, email = ? WHERE id = ?', (name, email, user_id))
            connection.commit()
