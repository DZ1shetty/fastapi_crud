...
class InstagramRepository:
    ...
    def follow_user(self, follower_id: int, followed_id: int) -> bool:
        with self._get_session() as session:
            ...
            if not self._user_exists(session, follower_id) or not self._user_exists(session, followed_id):
                return False
            existing = session.query(Follow).filter(Follow.follower_id == follower_id, Follow.followed_id == followed_id).first()
            if existing:
                return False
    ...
    def unfollow_user(self, follower_id: int, followed_id: int) -> bool:
        with self._get_session() as session:
            follow = session.query(Follow).filter(Follow.follower_id == follower_id, Follow.followed_id == followed_id).first()
            if not follow:
                return False
    ...
    def get_feed(self, user_id: int, limit: int = 20):
        with self._get_session() as session:
            ...
            return session.query(Post).filter(
                or_(Post.user_id == user_id, Post.user_id.in_(followed_ids))
